...
func TestNormalizeNativeMessagesBody_MovesSystemRolesToTopLevel(t *testing.T) {
	body := []byte(`{
		"model": "claude-opus-4.6",
		"system": [{"type":"text","text":"top","cache_control":{"type":"ephemeral"}}],
		"messages": [
			{"role":"system","content":"first"},
			{"role":"system","content":[{"type":"text","text":"second","cache_control":{"type":"ephemeral"}}]},
			{"role":"user","content":"hello"}
		],
		"max_tokens": 16
	}`)

	normalized, err := normalizeNativeMessagesBody(body, "", false)
	if err != nil {
	...
	want := []string{"top", "first", "second"}
	for i, raw := range system {
		block := raw.(map[string]interface{})
		if block["type"] != "text" || block["text"] != want[i] {
			t.Fatalf("system[%d] = %+v, want text %q", i, block, want[i])
		}
	}
	if _, ok := system[0].(map[string]interface{})["cache_control"]; !ok {
		t.Fatal("existing top-level system cache_control was not preserved")
	}
	if _, ok := system[2].(map[string]interface{})["cache_control"]; !ok {
		t.Fatal("in-message system cache_control was not preserved")
	}
}

func TestNormalizeNativeMessagesBody_RejectsNonTextSystemBlock(t *testing.T) {
	body := []byte(`{
		"model":"claude-opus-4.6",
		"messages":[
	...
	if _, err := normalizeNativeMessagesBody(body, "", false); err == nil {
		t.Fatal("expected a non-text system block to return an error")
	}
}

func TestInspectTopLevelFields_DetectsSystemMessageRole(t *testing.T) {
	body := []byte(`{"messages":[{"role":"system","content":"rules"},{"role":"user","content":"hi"}]}`)
	info := inspectTopLevelFields(body)
	if !info.HasSystemMessageRole {
		t.Fatal("expected HasSystemMessageRole=true")
	}
}
